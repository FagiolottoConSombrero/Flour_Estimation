...
# Assumo che questi siano già nel tuo progetto (come nel tuo script originale):
# from your_project import LLP, make_llp_loaders, set_seed, llp_kl_patch_loss, llp_kl_bag_loss
...
def test(
    ...
    # ============================
    conf_leak_sum = torch.zeros(NUM_CLASSES, NUM_CLASSES, dtype=torch.float64)  # [K,K]
    conf_leak_count = torch.zeros(NUM_CLASSES, dtype=torch.float64)            # [K]

    # ============================
    # SOTTOSTIMA (MISSING MASS): presente -> quanto manca rispetto al GT
    # ============================
    miss_sum = torch.zeros(NUM_CLASSES, dtype=torch.float64)
    miss_count = torch.zeros(NUM_CLASSES, dtype=torch.float64)
    ...
    # ============================
    # CONFUSIONE (LEAKAGE) + plot
    # ============================
    conf_leak_mean = conf_leak_sum.clone()
    for k in range(NUM_CLASSES):
        if conf_leak_count[k] > 0:
            conf_leak_mean[k] /= conf_leak_count[k]

    print("\n=== CONFUSIONE (LEAKAGE): farina presente -> massa media su farine assenti ===")
    for k in range(NUM_CLASSES):
        row = conf_leak_mean[k].numpy().copy()
        row[k] = 0.0  # ignora diagonale
        top = row.argsort()[::-1][:3]
        if conf_leak_count[k] > 0:
            print(f"\nPresente {FLOUR_ORDER[k]}: leakage medio su assenti (top-3)")
            for j in top:
                if row[j] > 0:
                    print(f"  -> {FLOUR_ORDER[j]} : {row[j]:.4f}")

    plt.figure(figsize=(6, 5))
    plt.imshow(conf_leak_mean.numpy(), interpolation="nearest")
    plt.colorbar(label="massa media su farine assenti")
    plt.xticks(range(NUM_CLASSES), FLOUR_ORDER)
    plt.yticks(range(NUM_CLASSES), FLOUR_ORDER)
    plt.title("Confusione (leakage): presente -> assente")

    for i in range(NUM_CLASSES):
        for j in range(NUM_CLASSES):
            v = conf_leak_mean[i, j].item()
            if v > 0:
                plt.text(j, i, f"{v:.2f}", ha="center", va="center", fontsize=8)

    plt.tight_layout()
    plt.savefig(f"{save_path}/confusion_leakage.png", dpi=300)
    plt.close()

    # ============================
    # SOTTOSTIMA (MISSING MASS)
    # ============================
    miss_mean = miss_sum / torch.clamp(miss_count, min=1.0)

    print("\n=== SOTTOSTIMA media (missing mass) per farina presente ===")
    for k in range(NUM_CLASSES):
        if miss_count[k] > 0:
            print(f"{FLOUR_ORDER[k]}: {miss_mean[k].item():.4f}")
